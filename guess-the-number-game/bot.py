...
def get_random_number() -> int:
    """ Return a random integer between 1 and 100.
        Вернуть случайное целое число от 1 до 100. """
...
async def process_positive_answer(message: Message):
    """ Process the user's consent to play the game.
        Обработать согласие пользователя сыграть в игру.
    """
...
async def process_negative_answer(message: Message):
    """ Handle the user's refusal to play the game.
        Обработать отказ пользователя сыграть в игру.
    """
    if not users[message.from_user.id]['in_game']:
        await message.answer(
...
async def process_numbers_answer(message: Message):
    """ Process the user sending numbers from 1 to 100.
        Обработать отправку пользователем чисел от 1 до 100.
    """
    if users[message.from_user.id]['in_game']:
...
async def process_other_answers(message: Message):
    """ Process any remaining messages.
        Обработать остальные любые сообщения.
    """
    if users[message.from_user.id]['in_game']:
        await message.answer(
